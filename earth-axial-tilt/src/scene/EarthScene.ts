...
import { CLASSIC_ORBIT, orbitKey, orbitalState, type OrbitParameters } from '../physics/orbit';
...
export class EarthScene {
  ...
  setState(next: Partial<SceneState>): void {
    ...
    const axisRotation = new THREE.Quaternion().setFromAxisAngle(
      new THREE.Vector3(0, 1, 0), THREE.MathUtils.degToRad(this.state.orbit.axisLongitude));
    const tiltRotation = new THREE.Quaternion().setFromAxisAngle(
      new THREE.Vector3(1, 0, 0), -THREE.MathUtils.degToRad(this.state.tilt));
    const spinRotation = new THREE.Quaternion().setFromAxisAngle(
      new THREE.Vector3(0, 1, 0), THREE.MathUtils.degToRad(this.state.rotation));
    this.earthGroup.quaternion.copy(axisRotation).multiply(tiltRotation).multiply(spinRotation);
    const orbitChanged = orbitKey(previous.orbit) !== orbitKey(this.state.orbit);
    ...
    if (seasonChanged || rotationChanged || next.mode !== undefined) {
      ...
      this.instantMesh.material.uniforms.relativeFlux.value = orbitalState(this.state.day, this.state.orbit).relativeFlux;
    }
  ...
  setComparison(value: { tilt: number; thermal: ThermalSolution | null; orbit: OrbitParameters } | null): void {
    if (value?.tilt === this.comparison?.tilt && value?.thermal === this.comparison?.thermal &&
      (!value || !this.comparison || orbitKey(value.orbit) === orbitKey(this.comparison.orbit))) return;
  ...
  private comparisonState(): SceneState {
    return { ...this.state, tilt: this.comparison!.tilt, thermal: this.comparison!.thermal, orbit: this.comparison!.orbit };
  }
    ...
    try {
      for (const view of views) {
        ...
        this.canvas.dataset[view.side === 'A' ? 'viewA' : 'viewB'] = JSON.stringify({
          tilt: this.state.tilt, day: this.state.day, rotation: this.state.rotation, orbit: this.state.orbit, axis,
  ...
  private syncOrbitLayout(): void {
    ...
    this.orbitOverview.setOrbit(this.state.orbit);
  ...
  private updateAxis(): void {
    ...
    if (axis) {
      axis.quaternion
        .setFromAxisAngle(new THREE.Vector3(0, 1, 0), THREE.MathUtils.degToRad(this.state.orbit.axisLongitude))
        .multiply(new THREE.Quaternion().setFromAxisAngle(
          new THREE.Vector3(1, 0, 0), -THREE.MathUtils.degToRad(this.state.tilt),
        ));
    }
    this.tiltArc.rotation.y = THREE.MathUtils.degToRad(this.state.orbit.axisLongitude);
  ...
  private updateDataLayer(): void {
    ...
    const cacheKey = `${this.state.mode}:${this.state.tilt}:${this.state.day}:${this.state.temperatureModel}:${this.state.heatDepth}:${orbitKey(this.state.orbit)}`;
    ...
    for (let i = 0; i < positions.count; i += 1) {
      ...
      if (this.state.mode === 'insolation') {
        t = THREE.MathUtils.clamp(
          dailyMeanInsolation(latitude, this.state.day, this.state.tilt, this.state.orbit) / SOLAR_CONSTANT,
          0,
          1,
        );
