...
export function createInstantSolarLayer(geometry: THREE.SphereGeometry): THREE.Mesh<THREE.SphereGeometry, THREE.ShaderMaterial> {
  ...
  const material = new THREE.ShaderMaterial({
    uniforms: { fluxRatio: { value: 1 }, sunLocal: { value: new THREE.Vector3(1, 0, 0) }, ramp: { value: colors } },
    vertexShader: `
    ...
    fragmentShader: `
      ...
      uniform float fluxRatio;
      uniform vec3 ramp[9];
      varying vec3 surfaceNormal;
      void main() {
        float solar = max(0.0, dot(normalize(surfaceNormal), sunLocal));
        float position = clamp(solar * fluxRatio, 0.0, 1.0) * 8.0;
