...
import { CLASSIC_ORBIT, orbitalState, type OrbitParameters } from './orbit';
...
export function rotatingSunDirection(day: number, tilt: number, rotation: number,
...
export function subsolarLongitude(day: number, tilt: number, rotation: number,
  orbit: OrbitParameters = CLASSIC_ORBIT): number | null {
...
export function solarMoment(
  ...
  return {
    ...
    insolation: SOLAR_CONSTANT * orbitalState(day, orbit).relativeFlux * Math.max(0, cosine),
...
export function insolationAtSolarHour(latitude: number, day: number, tilt: number, hours: number,
  ...
  return cosine < HORIZON_EPSILON ? 0 : SOLAR_CONSTANT * orbitalState(day, orbit).relativeFlux * Math.min(1, cosine);
}

export interface DiurnalPoint { hour: number; insolation: number }

export function diurnalProfile(latitude: number, day: number, tilt: number,
  orbit: OrbitParameters = CLASSIC_ORBIT): DiurnalPoint[] {
