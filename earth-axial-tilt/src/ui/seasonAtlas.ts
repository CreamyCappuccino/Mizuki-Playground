...
export class SeasonAtlas {
    ...
    constructor(private readonly actions: AtlasActions, bindLauncher = true) {
        ...
        const commit = () => {
            ...
            if (value === null) {
                ...
                this.el('atlas-input-status').textContent = tr('Enter 0–90°. The previous angle was kept.');
            }
            else {
                this.el('atlas-input-status').textContent = '';
        ...
        this.plot.addEventListener('pointerup', event => {
            ...
            this.gesture = null;
            if (!start || start.moved || start.id !== event.pointerId || Math.hypot(event.clientX - start.x, event.clientY - start.y) > 8 || !this.layout)
                return;
    ...
    update(snapshot: AtlasSnapshot): void {
        ...
        if (box.width > 0 && box.height > 0 && (key !== this.fieldKey || layoutKey !== this.layoutKey)) {
            if (key !== this.fieldKey) {
                this.field = buildAtlasField(config);
                this.fieldKey = key;
            }
    ...
    private paint(config: AtlasConfig): void {
        ...
        this.canvas.width = Math.round(l.width * dpr);
        ...
        if (this.field) {
            ...
            raster.width = field.columns;
            raster.height = field.rows;
    ...
    private moveCursor(day: number, latitude: number): void {
        ...
        cross?.setAttribute('cx', String(x));
        cross?.setAttribute('cy', String(y));
    }
}
