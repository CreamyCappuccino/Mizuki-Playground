...
export function bindContextHelp(): void {
  ...
  const tip = document.createElement('div'); tip.id = 'help-tooltip'; tip.className = 'help-tooltip';
  ...
  const render = () => {
    ...
    const h = HELP[active]; title.textContent = pick(h.title); body.replaceChildren();
    for (const [heading, value] of [['', h.meaning], [t('Try this'), h.experiment], [t('Look for'), h.observe], [t('Keep in mind'), h.limit]] as const) {
      if (heading) { const label = document.createElement('h3'); label.textContent = heading; body.append(label); }
      const p = document.createElement('p'); p.textContent = pick(value); body.append(p);
    }
  };
  const buttons: HTMLButtonElement[] = [];
  document.querySelectorAll<HTMLElement>('[data-help]').forEach(anchor => {
    const key = anchor.dataset.help!; if (!HELP[key]) return;
    const button = document.createElement('button'); button.type = 'button'; button.className = 'help-button';
    button.textContent = '?'; button.dataset.helpTopic = key; button.setAttribute('aria-haspopup', 'dialog');
    button.setAttribute('aria-controls', 'context-help');
    // Sibling controls, never nested inside a label/button/link.
    anchor.insertAdjacentElement('afterend', button); buttons.push(button);
    const preview = () => {
      hide(); if (dialog.open) return; owner = button;
      timer = window.setTimeout(() => {
        const parent = document.querySelector('dialog[open]') ?? document.body; parent.append(tip);
        tip.textContent = `${pick(HELP[key].title)} — ${pick(HELP[key].meaning)}`; tip.hidden = false;
        button.setAttribute('aria-describedby', tip.id);
        const b = button.getBoundingClientRect(), w = Math.min(340, window.innerWidth - 24);
        tip.style.width = `${w}px`; tip.style.left = `${Math.max(12, Math.min(b.left, window.innerWidth - w - 12))}px`;
    ...
    button.addEventListener('pointerenter', e => { if (e.pointerType === 'mouse') preview(); });
    button.addEventListener('focus', preview);
    button.addEventListener('pointerleave', () => { clearTimeout(timer); timer = window.setTimeout(hide, 150); });
    button.addEventListener('blur', hide);
    button.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); hide(); owner = button; active = key; render(); dialog.showModal(); });
  });
  tip.addEventListener('pointerenter', () => clearTimeout(timer)); tip.addEventListener('pointerleave', hide);
  close.addEventListener('click', () => dialog.close());
  dialog.addEventListener('close', () => { hide(); owner?.focus({ preventScroll: true }); });
  ...
  const localize = () => { buttons.forEach(b => b.setAttribute('aria-label', `${t('How to explore')}: ${pick(HELP[b.dataset.helpTopic!].title)}`)); if (dialog.open) render(); hide(); };
  onLanguageChange(localize); localize();
}
