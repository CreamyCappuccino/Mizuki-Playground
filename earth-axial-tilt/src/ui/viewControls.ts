...
export function bindViewControls(actions: ViewActions): void {
  const get = <T extends HTMLElement>(id: string): T => {
    ...
    if (!element) throw new Error(`Missing view control: ${id}`);
    return element as T;
  };
  ...
  let frame = 0;
  try { quality.value = parseVisualQuality(localStorage.getItem('earth-lab:quality')); }
  catch { quality.value = 'high'; }
  actions.quality(parseVisualQuality(quality.value));
  quality.addEventListener('change', () => {
    const value = parseVisualQuality(quality.value); quality.value = value;
    actions.quality(value);
    try { localStorage.setItem('earth-lab:quality', value); } catch { /* storage may be unavailable */ }
  });
  lights.addEventListener('change', () => actions.lights(lights.checked));
  const setFocus = (value: boolean) => {
    if (focused === value) return;
    cancelAnimationFrame(frame);
    if (value) { scrollX = window.scrollX; scrollY = window.scrollY; }
    focused = value;
    root.dataset.focusView = String(focused);
    focusButton.textContent = focused ? 'Show controls' : 'Focus view';
    ...
    frame = requestAnimationFrame(() => {
      actions.refresh();
      if (!focused) window.scrollTo(scrollX, scrollY);
    });
  };
  ...
  const status = () => {
    ...
    note.textContent = day === 'error' ? 'Day map unavailable — plain globe fallback. Science controls still work.'
      : night === 'error' ? 'Night map unavailable — lights omitted. Science controls still work.'
        : day === 'ready' && night === 'ready' ? 'Local 4K maps ready · fixed decorative imagery, not live weather.'
          : 'Loading local Earth maps…';
  };
  canvas.addEventListener('visualstatus', status); status();
}
