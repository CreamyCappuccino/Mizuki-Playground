# Experiment state format — schema 1

A reproducible scientific experiment is distinct from display preferences, a cached solution and civil time.

## URL

A complete link contains an app origin/path and a `#lab=1&…` fragment. The serializer removes arbitrary query parameters and credentials. Sharing is user-initiated; playback does not continuously replace browser history. Existing non-experiment fragments are ignored.
...
UTF-8 JSON envelope: `application: "earth-axial-tilt"`, `version: 1`, `state: ExperimentState`. Input size is checked before reading; the same validator applies. Exported files contain no image, user account, location access request or thermal array.
...
No locale, text size, quality, Basic/All, Focus, camera pose or playback is imported. The 1.0 circular-year/time definitions still apply. Fast/Slow presets are successive runs, not unequal A/B heat capacities. Formal 1.0 device acceptance and public deployment remain separate gates.
