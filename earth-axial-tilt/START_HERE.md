...
2. `docs/V0.8.md`：直近の変更と確認項目。以前の全会話を読む必要はない。
3. `docs/GUIDE.ja.md`：利用者の操作と、何を読み取るか。
4. 変更する分野のファイルだけ（下表）。物理を変更するときだけ `docs/SCIENCE.md` を精読。
...
- `Play day`は日付を固定して自転。`Play year`は自転角を固定して公転位相を動かす。両方同時再生ではない。
...
各版のREADME・科学説明・変更履歴・画像の出典は既に残っていた。一方、現状への入口と操作ガイドがなく、過去版の制限と現在の機能が混ざりやすかった。このファイルを入口とし、操作をGUIDE、最新変更をV0.8、科学の仕様をSCIENCE、残作業を下欄へ分離した。

## 残っている候補（未着手を完成と混同しない）

Dual Earth、独立した雲レイヤー、実機Safari確認、golden画像との回帰比較、package-lockとnpm ci、lint、公開deployment。さらに先は海陸／氷のフィードバック、離心率・歳差、別惑星。日英化・解説・公転俯瞰はv0.8の対象で、これらの先送り項目とは別。

## この文書の更新

次の版で「まずここだけ」と残作業を更新し、直近のV0.xへリンクする。検証runと正確なcommitはGitHubの履歴／CIと各リリースの報告を正本にし、古い会話の番号だけに依存しない。
